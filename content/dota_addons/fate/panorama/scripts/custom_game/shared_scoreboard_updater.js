...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	if ( playerInfo )
	{
		...
		var picked = CustomNetTables.GetTableValue("nselection", "picked") || {};
		var skin = CustomNetTables.GetTableValue("nselection", "skinselect") || CustomNetTables.GetTableValue("draft", "skinselect") || {};
		var selectedPortrait = CustomNetTables.GetTableValue("nselection", "select_bar") || {};
		var skin_num = ""
		if ( playerPortrait )
		{
			if ( playerInfo.player_selected_hero !== "" )
			{
				...
				if (Game.GameStateIs( DOTA_GameState.DOTA_GAMERULES_STATE_PRE_GAME))
				{
					if ( !!picked[playerId]) {
						...
						if (skin[picked[playerId]] > 0) {
							skin_num = "_" + skin[picked[playerId]]
						}
						playerPortrait.SetImage( "s2r://panorama/images/custom_game/portrait/" + picked[playerId] + skin_num + "_png.vtex" );
						playerPanel.SetHasClass( "player_dead", false );
					} else if (!!selectedPortrait[playerId] && isTeammate) {
						playerPortrait.SetImage( "s2r://panorama/images/custom_game/portrait/" + selectedPortrait[playerId] + "_png.vtex" );
						playerPanel.SetHasClass( "player_dead", true );
					};
				} else {
					if (skin[playerInfo.player_selected_hero] > 0) {
						skin_num = "_" + skin[playerInfo.player_selected_hero]
					}
                	playerPortrait.SetImage("s2r://panorama/images/custom_game/portrait/" + playerInfo.player_selected_hero + skin_num + "_png.vtex");
				};
			}
			else
			{
