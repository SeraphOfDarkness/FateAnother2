...
//g_GameConfig.duration = [186,327,138,149,181,143,184,366,239,141,184,213,180,166,156,173,143];
...
var mode_panel = $.GetContextPanel().FindChildTraverse("ModeList");
mode_panel.visible = false;
mode_panel.FindChildTraverse("ModeButton2").visible = false;

var grail_panel = $.GetContextPanel().FindChildTraverse("GrailList");
grail_panel.visible = false;

var balance_panel = $.GetContextPanel().FindChildTraverse("BalanceList");
...
function OnAutoAssignPressed()
{
	...
	if (g_GameBalance == true) {
		$.Msg('auto balance shufle');
		GameEvents.SendCustomGameEventToServer("balance_shuf", {
            balance: true,
        });
	} else {
...
function IsAutoBalance(data)
{
	var balanze = data.balance 
	g_GameBalance = balanze
	//Game.PlayerJoinTeam( DOTATeam_t.DOTA_TEAM_NOTEAM );
}

function AutoBalanze(data)
{
	var balance = data;
	var playerId = Players.GetLocalPlayer();

	if (!!data[playerId]) {
		$.Msg('player ' + playerId + ' team ' + data[playerId]);
		Game.PlayerJoinTeam( data[playerId] );
	};
}
...
function SendVotes( )
{
	...
	var modeListener = CustomNetTables.GetTableValue("mode", "mode") || {};	
	if (modeListener !== "" && modeListener["mode"] == true) {
		var ModeVoted = 1

		if ($( "#ModeButton1" ).checked == true) {
			ModeVoted = 1
		} else if ($( "#ModeButton2" ).checked == true) {
			ModeVoted = 2
		};
		$.Msg("game mode : " + ModeVoted);
		//$.Msg('host ' + playerInfo.player_has_host_privileges);
		if (playerInfo.player_has_host_privileges == true) {
			GameEvents.SendCustomGameEventToServer("game_vote", {player: Players.GetLocalPlayer(), ModeVoted: ModeVoted})
		};
	};

	var grailListener = CustomNetTables.GetTableValue("mode", "grail_mode") || {};
	if (grailListener !== "" && grailListener["mode"] == true) {
		var GrailVoted = 1

		if ($( "#GrailButton1" ).checked == true) {
			GrailVoted = 1
		} else if ($( "#GrailButton2" ).checked == true) {
			GrailVoted = 2
		};
		$.Msg("grail mode : " + ModeVoted)
		GameEvents.SendCustomGameEventToServer("grail_vote", {player: Players.GetLocalPlayer(), GrailVoted: GrailVoted})
	};

	var balanceListener = CustomNetTables.GetTableValue("mode", "balance_mode") || {};	
	if (balanceListener !== "" && balanceListener["mode"] == true) {
		if (playerInfo.player_has_host_privileges == true) {
			var BalanceVoted = 1;

			if ($( "#BalanceButton1" ).checked == true) {
				BalanceVoted = 1;
			} else if ($( "#BalanceButton2" ).checked == true) {
				BalanceVoted = 2;
			} else if ($( "#BalanceButton3" ).checked == true) {
				BalanceVoted = 3;
			};
			$.Msg("ban mode : " + BalanceVoted)
			GameEvents.SendCustomGameEventToServer("balance_vote", {player: Players.GetLocalPlayer(), BalanceVoted: BalanceVoted})
		};
	};
}

function DMEnable(data)
{
	var mode = data.dm;
	if (mode == true) {
		mode_panel.FindChildTraverse("ModeButton2").visible = true;
		$( "#ModeButton1" ).checked = true;
		$( "#ModeButton2" ).checked = false;
	}
}
...
(function()
{
	...
	if (modeListener !== "" && modeListener["mode"] == true) {
		...
		$( "#ModeButton1" ).checked = true;
		var mode2Listener = CustomNetTables.GetTableValue("mode", "availablemode") || {};
		if (mode2Listener["availablemode"] == true) {
			mode_panel.FindChildTraverse("ModeButton2").visible = true;
			$( "#ModeButton1" ).checked = true;
			$( "#ModeButton2" ).checked = false;
		};
	...
	var grailListener = CustomNetTables.GetTableValue("mode", "grail_mode") || {};
	if (grailListener !== "" && grailListener["mode"] == true) {
		grail_panel.visible = true;
		$( "#GrailButton1" ).checked = true;
	};
	...
	GameEvents.Subscribe( "auto_balanze", IsAutoBalance);
	GameEvents.Subscribe( "auto_balance", AutoBalanze);
	GameEvents.Subscribe( "availablemode", DMEnable);
})();
