...
LinkLuaModifier("modifier_vortigern_ferocity", "abilities/artoria_alter/modifiers/modifier_vortigern_ferocity", LUA_MODIFIER_MOTION_NONE)
...
function OnDexHit(keys)
	...
	local target = keys.target
	local ply = caster:GetPlayerOwner()
	if caster.IsDarklightAcquired then keys.Damage = keys.Damage + 400 end
