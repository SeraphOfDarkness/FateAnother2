...
LinkLuaModifier("modifier_tawrich_crit", "abilities/angra_mainyu/modifiers/modifier_tawrich_crit", LUA_MODIFIER_MOTION_NONE)
...
function OnBashSuccess(keys)
	...
	if not caster:HasModifier("modifier_murderous_instinct_bash_checker") then
...
function OnTZStart(keys)
    ...
    if not IsImmuneToSlow(keys.target) and not target:IsMagicImmune() then 
    	keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_tawrich_slow", {}) 
    end
end
