...
function DraftSelectioN:StartBanPhase()	
	...
	if IsInToolsMode() then 
		self.BanOrder[self.Draft.ban_order] = {teamNum = "1", playerId = 0}
	end	
...
function DraftSelectioN:AssignHero(playerId, hero, skin)
    ...
    if oldHero == nil then 
    	SendChatToPanorama('Player ' .. playerId .. ' : Summon medium Servant ' .. hero .. ' has not founded.')
    	oldHero = EntIndexToHScript(ddt.hhero) or EntIndexToHScript(ddt.io)
    	SendChatToPanorama('Player ' .. playerId .. ' : Changing summon medium ' .. oldHero .. '.')
    end
    if not oldHero.IsHeroSpawn then
		oldHero:SetRespawnsDisabled(true)
		PlayerResource:ReplaceHeroWith(playerId, hero, 3000, 0)
		UTIL_Remove(oldHero)
		SendChatToPanorama('Player ' .. playerId .. ' : Servant ' .. hero .. ' has been summoned.')
