...
function OnGKStart(keys)
	...
	if caster.IsQuickdrawAcquired and not caster:HasModifier("modifier_quickdraw_cooldown") then 
		caster:SwapAbilities("false_assassin_gatekeeper", "false_assassin_quickdraw", false, true) 
		Timers:CreateTimer(5, function() return caster:SwapAbilities("false_assassin_gatekeeper", "false_assassin_quickdraw", true, false)   end)
	end
...
function OnHeartStart(keys)
	...
	--[[else
		...
		caster:FindAbilityByName("false_assassin_gatekeeper"):StartCooldown(keys.GCD) 
...
function OnHeartDamageTaken(keys)
	...
	if damageTaken > keys.Threshold and caster:GetHealth() ~= 0 then
		...
		if (caster:GetAbsOrigin()-target:GetAbsOrigin()):Length2D() < 1500 and not target:IsInvulnerable() and caster:GetTeam() ~= target:GetTeam() then
			...
			-- cooldown
			ReduceCooldown(caster:FindAbilityByName("false_assassin_gatekeeper"), cdr)
...
function OnPCEnd(keys)
    ...
    for i, v in ipairs(abilities) do
        ...
        if ability:GetAbilityName() == "false_assassin_quickdraw" and ability:IsCooldownReady() then 
...
function OnQuickdrawHit(keys)
	...
	local damage = 500 + keys.caster:GetAgility() * 17
...
function OnWBStart(keys)
	...
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_wb_baseattack_bonus_damage", {})
...
function OnTGStart(keys)
	...
	if caster:GetName() == "npc_dota_hero_juggernaut" then
		...
		-- 1.27d caster:FindAbilityByName("false_assassin_gatekeeper"):StartCooldown(keys.GCD) 
	...
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_tg_baseattack_bonus_damage", {})
...
function FACheckCombo(caster, ability)
	if caster:GetStrength() >= 29.1 and caster:GetAgility() >= 29.1 then
		if ability == caster:FindAbilityByName("false_assassin_gatekeeper") and caster:FindAbilityByName("false_assassin_heart_of_harmony"):IsCooldownReady() and caster:FindAbilityByName("false_assassin_hiken_enbu"):IsCooldownReady() and not caster:HasModifier("modifier_tsubame_mai_cooldown") then
			caster:SwapAbilities("false_assassin_heart_of_harmony", "false_assassin_hiken_enbu", false, true) 
			Timers:CreateTimer({
				endTime = 3,
				callback = function()
				caster:SwapAbilities("false_assassin_heart_of_harmony", "false_assassin_hiken_enbu", true, false) 
			end
			})			
		end
	end
