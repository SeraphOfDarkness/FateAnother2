...
function muramasa_dash_wrapper(ability)
	function ability:OnSpellStart()
		...
		if caster:HasModifier("modifier_muramasa_marble") then
			distance = self:GetSpecialValueFor("range_marble")
		end
		...
		StartAnimation(caster, {duration= 0.4, activity=ACT_DOTA_CAST_ABILITY_2, rate= 2})
		...
		Timers:CreateTimer(0.3,function()
			...
			StartAnimation(caster, {duration= 0.4, activity=ACT_DOTA_CAST_ABILITY_8, rate= 2})
	...
	function ability:GetCastRange()
		local caster = self:GetCaster()
		local distance = self:GetSpecialValueFor("range")
		local distance_marble = self:GetSpecialValueFor("range_marble")
		local ability = self

		if caster:HasModifier("modifier_muramasa_marble") then
			return distance_marble
		else
			return distance
		end
	end
...
modifier_muramasa_combo_window = class({})
function modifier_muramasa_combo_window:OnCreated(args)
	if IsServer() then
...
function modifier_muramasa_combo_window:OnDestroy()
	if IsServer() then
		local caster = self:GetParent()
		if caster.is3000Acquired then 
...
function modifier_muramasa_combo_window:RemoveOnDeath()
	return true
end
function modifier_muramasa_combo_window:GetAttributes()
