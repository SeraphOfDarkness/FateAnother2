    ...
    for i=1, #ubwTargets do
        if IsValidEntity(ubwTargets[i]) then
            if ubwTargets[i]:GetName() ~= "npc_dota_ward_base" then
                ...
                if IsIsekaiAbuser(ubwTargets[i])then
                    giveUnitDataDrivenModifier(caster, ubwTargets[i], "modifier_isekai_check", 15)
                    giveUnitDataDrivenModifier(caster, ubwTargets[i], "modifier_isekai_abuser", 20)
                end
