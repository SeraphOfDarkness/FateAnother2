	...
	for i = 1,total_hit do 
		Timers:CreateTimer(0.25 * i, function()  
			if caster:IsAlive() and target:IsAlive() then
				PerformSlash(caster, ability, target)
						...
						if target:HasModifier("modifier_jtr_whitechapel_female_enemies") and IsFemaleServant(target) then
							...
							if RandomInt(1, 100) <= curse_chance then 
								if IsUnExecute(target) then
									...
									local curseIndex = ParticleManager:CreateParticle("particles/econ/items/dark_willow/dark_willow_ti8_immortal_head/dw_crimson_ti8_immortal_cursed_crownmarker_steam.vpcf", PATTACH_ABSORIGIN_FOLLOW, target )
...
function PerformSlash(caster,ability,target)
