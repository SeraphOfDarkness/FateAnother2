...
function OnShapeShiftStart(keys)
	...
	Attributes:ModifyBonuses(illusion[1])
	caster.ShapeShiftIllusion:SetHealth(caster:GetHealthPercent() * caster.ShapeShiftIllusion:GetMaxHealth())
	-- Only GetMaxMana but no SetMaxMana wth valve
	caster.ShapeShiftIllusion:SetMana(caster:GetManaPercent() * caster.ShapeShiftIllusion:GetMaxMana())

	local move = {
		UnitIndex = illusion[1]:entindex(), 
