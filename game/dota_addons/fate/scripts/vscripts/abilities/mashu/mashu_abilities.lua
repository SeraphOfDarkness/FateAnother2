...
function OnProtectProc(keys)
    ...
    local red_cooldown = ability:GetSpecialValueFor("damage_proc")
...
function OnMashuTauntThink(keys)
	...
	forcemove.UnitIndex = unit:entindex()
	forcemove.Position = caster:GetAbsOrigin() 
	unit:Stop()
	ExecuteOrderFromTable(forcemove) 
    local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_doom_bringer/doom_bringer_lvl_death.vpcf", PATTACH_ABSORIGIN_FOLLOW, unit)
...
function OnBunkerBoltThink(vLocation)
