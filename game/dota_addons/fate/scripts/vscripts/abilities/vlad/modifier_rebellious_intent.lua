...
function modifier_rebellious_intent:GetModifierAttackSpeedBonus_Constant()
  return self:GetStackCount()*self:GetAbility():GetSpecialValueFor("att_speed_stack")
end
...
if IsServer() then
  ...
  function modifier_rebellious_intent:OnStackCountChanged(iStackCount)
    ...
    if parent:IsAlive() then
      ...
      local difference_to_heal = ((current_hp + 7 * str_per_stack) / (max_hp + 7 * str_per_stack)) * max_hp - current_hp
  ...
  function modifier_rebellious_intent:OnRemoved()
    ...
    if parent:IsAlive() then 
      ...
      for i=1, self:GetStackCount(), 1 do
        local current_hp = parent:GetHealth()
        local max_hp = parent:GetMaxHealth()
        local difference_to_subtract = ((current_hp + 7 * str_per_stack) / (max_hp + 7 * str_per_stack)) * max_hp - current_hp
