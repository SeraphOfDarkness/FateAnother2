  		...
  		if IsValidEntity(v) and not v:IsNull() and v:HasModifier("modifier_vlad_bleed") then
  			...
  			if count > 0 then
        	    ...
        	    if caster.IsBloodletterAcquired then
				  	...
				  	caster:SetModifierStackCount("modifier_transfusion_blood_power", caster, currentStack + 1)
				end
        	else
...
function OnImpaleStart(keys)
	...
	local stun_min = ability:GetSpecialValueFor("stun_min")
  	...
  	local spikeFX = ParticleManager:CreateParticle("particles/custom/vlad/vlad_ip_prespike.vpcf", PATTACH_CUSTOMORIGIN, caster)
  	ParticleManager:SetParticleControl(spikeFX,0,point)
  	ParticleManager:SetParticleControl(spikeFX,3,point)
  	ParticleManager:SetParticleControl(spikeFX,4,Vector(radius, 0, 0))
	...
	if caster.IsBloodletterAcquired then
		if not caster:HasModifier("modifier_vlad_transfusion_self") then 
			...
			if caster:HasModifier("modifier_transfusion_blood_power") then
				...
				local bonus_dmg_per_bloodpower = ability:GetSpecialValueFor("modifier_transfusion_blood_power") / 100
				...
				if bloodpower > bloodpowercap then 
					...
					--[[caster:RemoveModifierByName("modifier_transfusion_blood_power")
					bleed:ApplyDataDrivenModifier(caster, caster, "modifier_transfusion_blood_power", {duration = bloodpowerduration})
					caster:SetModifierStackCount("modifier_transfusion_blood_power", caster, bloodpower - bloodpowercap)]]
				else 
					caster:RemoveModifierByName("modifier_transfusion_blood_power")
				end
			end
	...
	if caster.IsBloodletterAcquired then
		if not caster:HasModifier("modifier_vlad_transfusion_self") then 
			...
			if caster:HasModifier("modifier_transfusion_blood_power") then
				local bloodpower = caster:GetModifierStackCount("modifier_transfusion_blood_power", caster) or 0
				local bloodpowerduration = caster.BloodPower_duration
				local bonus_shield_per_bloodpower = ability:GetSpecialValueFor("modifier_transfusion_blood_power") / 100
				local bonus_dmg_per_bloodpower = ability:GetSpecialValueFor("modifier_transfusion_blood_power") / 100
				local bloodpowercap = caster.MasterUnit2:FindAbilityByName("vlad_attribute_bloodletter"):GetSpecialValueFor("bloodpower_cap")
				local bleed = caster:FindAbilityByName("vlad_transfusion_upgrade")
...
function OnLordOfExecutionStart(keys)
	...
	Timers:CreateTimer(0.15, function()
		...
		ParticleManager:SetParticleControlEnt( SpikeFieldFX,1 , caster,  PATTACH_ABSORIGIN, nil, caster:GetAbsOrigin(), false )
