...
function InvisibleAirPull(keys)
	...
	keys.caster.invisible_air_reach_target = true
...
function OnExcaliburHit(keys)
	...
	local target = keys.target 
	local ply = caster:GetPlayerOwner()
	if caster.IsExcaliburAcquired == true then keys.Damage = keys.Damage + 400 end
...
function AvalonDash(caster, attacker, counterdamage, ability)
	...
	Timers:CreateTimer({
		...
		-- Original function
		local targets = FindUnitsInRadius(caster:GetTeam(), targetPoint, nil, 250, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, FIND_ANY_ORDER, false)
		for k,v in pairs(targets) do
...
function StrikeAirPush(keys)
	...
	local totalDamage = 400 + keys.caster:FindAbilityByName("saber_invisible_air"):GetLevel() * 100
