...
function OnEHit(keys)
    ...
    damage = base_damage + (damage_increment * (shotNumber - 1))
...
function OnRHit(keys)
    ...
    if caster.IsOutlaw and caster.finalShot and caster.rHits == 3 then
        ...
        DoDamage(caster, target, damage + last_ammo_bonus, DAMAGE_TYPE_MAGICAL, 0, ability, false)
    else
        DoDamage(caster, target, damage, DAMAGE_TYPE_MAGICAL, 0, ability, false)
    end
...
function OnCombo(keys)
    ...
    local particleeff = ParticleManager:CreateParticle("particles/billy/billy_r_dust.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
    ParticleManager:SetParticleControl(particleeff, 1, caster:GetAbsOrigin() + Vector(0,0,10))
    local particleeff = ParticleManager:CreateParticle("particles/econ/items/dragon_knight/dk_2022_immortal/dk_2022_immortal_dragon_tail_dragon_impact_flare.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
    ParticleManager:SetParticleControl(particleeff, 1, caster:GetAbsOrigin() + Vector(0,0,10))
    local particleeff = ParticleManager:CreateParticle("particles/econ/items/dazzle/dazzle_ti6_gold/dazzle_ti6_shallow_grave_gold_glyph_flare.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
    ...
    Timers:CreateTimer(cast_delay, function()
        ...
        ParticleManager:SetParticleControl(particleeff, 0, caster:GetAbsOrigin() + Vector(0,0,10))
        local particleeff = ParticleManager:CreateParticle("particles/econ/events/ti10/hot_potato/disco_ball_death_flare.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
        ParticleManager:SetParticleControl(particleeff, 0, caster:GetAbsOrigin() + Vector(0,0,10))
        local particleeff = ParticleManager:CreateParticle("particles/billy/billy_combo_flare_self.vpcf", PATTACH_WORLDORIGIN, caster)
...
function OnComboHit(keys)
    ...
    if target and not target:IsNull() and target:IsAlive() then
        DoDamage(caster, target, damage, DAMAGE_TYPE_MAGICAL, 0, ability, false)    local particleeff = ParticleManager:CreateParticle("particles/econ/items/sniper/sniper_charlie/sniper_assassinate_impact_c_charlie.vpcf", PATTACH_CUSTOMORIGIN, target)
        ParticleManager:SetParticleControl(particleeff, 1, target:GetAbsOrigin() + Vector(0,0,60))
        local particleeff = ParticleManager:CreateParticle("particles/econ/items/sniper/sniper_charlie/sniper_assassinate_impact_blood_charlie.vpcf", PATTACH_CUSTOMORIGIN, target)
        ParticleManager:SetParticleControl(particleeff, 0, caster:GetAbsOrigin() + Vector(0,0,60))
        ParticleManager:SetParticleControl(particleeff, 1, target:GetAbsOrigin() + Vector(0,0,60))
