...
function modifier_atalanta_jump:PlayEffects()
    if not self.do_damage then
        ...
        if self.parent.WildBeastLogicAcquired then 
            radius = self:GetAbility():GetSpecialValueFor("knock_radius")
    	...
    	for _,enemy in ipairs(enemies2) do
            ...
            local knockback = { should_stun = false,
    	                                ...
    	                                duration = 0,
    	                                knockback_distance = self.parent.WildBeastLogicAcquired and -math.min((self.parent:GetAbsOrigin() - enemy:GetAbsOrigin()):Length2D(), self.radius) or 0,
    	                                knockback_height = self.parent.WildBeastLogicAcquired and 50 or 0,
