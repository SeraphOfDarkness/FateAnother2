...
function modifier_ishtar_w_dash:GetModifierIncomingDamage_Percentage()
	...
	if self:GetParent():HasModifier("modifier_ishtar_mana_burst_gem") then 
		reduction = reduction + (self.gem_use * self:GetAbility():GetSpecialValueFor("gem_reduction"))
	end
	return reduction
end
...
function modifier_ishtar_w_dash:UpdateHorizontalMotion(me, dt)
    ...
    for _,enemy in pairs (enemies) do
    	if not enemy:HasModifier("modifier_ishtar_w_damage_cooldown") then 
    		...
    		DoDamage(self.caster, enemy, self:GetAbility():GetSpecialValueFor("damage") + (self:GetAbility():GetSpecialValueFor("gem_damage") * self.gem_use), DAMAGE_TYPE_MAGICAL, DOTA_DAMAGE_FLAG_NONE, self, false)
    		if self.gem_use > 0 and not IsKnockbackImmune(enemy) and not IsImmuneToCC(enemy) then 
    			local knockback = { should_stun = true,
    	                                knockback_duration = 0.5,
    	                                duration = 0.1,
    	                                knockback_distance = self.gem_use * self:GetAbility():GetSpecialValueFor("gem_knock"),
    	                                knockback_height = 0,
    	                                center_x = position.x,
    	                                center_y = position.y,
    	                                center_z = position.z }
