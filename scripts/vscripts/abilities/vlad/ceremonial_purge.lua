		...
		Timers:CreateTimer(delay, function()
	      ...
	      --print(distance)
				...
				if distance < aoe_inner then
					...
					v:AddNewModifier(caster, v, "modifier_stunned", { Duration = stun_inner })					
	        		...
	        		v:AddNewModifier(caster,self,"modifier_ceremonial_purge_slow",{duration = slow_duration})
	       			v:AddNewModifier(caster, v, "modifier_stunned", { Duration = stun_outer })
	       			--giveUnitDataDrivenModifier(caster, v, "stunned", stun_outer)
					caster:AddBleedStack(v, false, 3)
				end
			end
		end)
	end
