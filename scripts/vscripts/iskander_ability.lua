...
LinkLuaModifier("modifier_gordius_wheel_temp_bandaid", "abilities/iskandar/modifiers/modifier_gordius_wheel_temp_bandaid", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_iskandar_chariot_fixed_ms", "abilities/iskandar/modifiers/modifier_iskandar_chariot_fixed_ms", LUA_MODIFIER_MOTION_NONE)


function OnChariotStart(keys)
	local caster = keys.caster
	if caster.IsRiding or caster:HasModifier("modifier_army_of_the_king_death_checker") then 
	...
	keys.ability:ApplyDataDrivenModifier(caster, caster, "modifier_gordius_wheel_data", {}) 
...
function OnChariotRide(keys)
	...
	caster.BonusChargeDamage = 0
   	...
   	Timers:CreateTimer(function() 
   		...
   		if caster.IsRiding and counter < 10 then 
	        ...
	        caster.BonusChargeDamage = counter * 50
	...
	if not caster:HasModifier("modifier_army_of_the_king_death_checker") then
		...
		Timers:CreateTimer(function()	
			if caster:HasModifier("modifier_gordius_wheel_data") then 
		...
		Timers:CreateTimer(2.49, function()	
			if caster:HasModifier("modifier_gordius_wheel_data") and caster:HasModifier("modifier_gordius_wheel_mitigation_tier1") then 
		...
		Timers:CreateTimer(5.49, function()	
			if caster:HasModifier("modifier_gordius_wheel_data") and caster:HasModifier("modifier_gordius_wheel_mitigation_tier2") then 
...
function OnChariotEnd(keys)
    ...
    caster:RemoveModifierByName("modifier_gordius_wheel_temp_bandaid")
end
...
function OnChariotChargeStart(keys)
	...
	Timers:CreateTimer("chariot_dash_damage", {
        ...
        local bonus_charge_damage = caster.BonusChargeDamage or 50
        ...
        for k,v in pairs(targets) do
			if v.ChariotChargeHit ~= true then 
				...
				Timers:CreateTimer(1.0, function()
           		...
           		DoDamage(caster, v, (keys.ChargeDamage * currentMS / 100) + bonus_charge_damage, DAMAGE_TYPE_MAGICAL, 0, keys.ability, false)
	...
	Timers:CreateTimer("chariot_dash", {
		...
		caster:RemoveModifierByName("modifier_gordius_wheel_data")
		caster:RemoveModifierByName("modifier_gordius_wheel_temp_bandaid")
		caster:RemoveModifierByName("modifier_iskandar_chariot_fixed_ms")
		caster.IsRiding = false
...
function OnAOTKStart(keys)
	...
	caster:SwapAbilities("iskander_gordius_wheel", "iskandar_arrow_bombard", false, true)
	if caster.IsBeyondTimeAcquired then
		...
		if IsValidEntity(aotkTargets[i]) and not aotkTargets[i]:IsNull() then
			if aotkTargets[i]:GetName() ~= "npc_dota_ward_base" then
				...
				if aotkTargets[i]:GetName() == "npc_dota_hero_bounty_hunter" or aotkTargets[i]:GetName() == "npc_dota_hero_riki" then
	                aotkTargets[i]:AddNewModifier(caster, ability, "modifier_inside_marble", { Duration = 12 })
	            end

	            if IsIsekaiAbuser(aotkTargets[i]) then
                    giveUnitDataDrivenModifier(caster, aotkTargets[i], "modifier_isekai_check", 12)
                    giveUnitDataDrivenModifier(caster, aotkTargets[i], "modifier_isekai_abuser", 20)
                end
...
function OnAOTKSoldierDeath(keys)
	...
	if keys.target and keys.target.IsAOTKSoldier then
		caster.AOTKSoldierCount = caster.AOTKSoldierCount - 1
...
function EndAOTK(caster)
	...
	caster:SwapAbilities("iskander_gordius_wheel", "iskandar_arrow_bombard", true, false)
