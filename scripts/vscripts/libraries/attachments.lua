...
require('abilities/lishuwen/modifiers/modifier_pc_invis')
--require('abilities/vlad/modifier_q_used')
  ...
  if IsInToolsMode() then
    ...
    if src:sub(2):find("(.*dota 2 beta[\\/]game[\\/]dota_addons[\\/])([^\\/]+)[\\/]") then
      ...
      if IsInToolsMode() then
        ...
        Convars:RegisterCommand( "tooltip_test", Dynamic_Wrap(Attachments, 'ActivateTooltipTest'), "Test Tooltip Setup", FCVAR_CHEAT )
      end
    else
...
function Attachments:ActivateTooltipTest()
  local ply = Convars:GetCommandClient()
  CustomGameEventManager:Send_ServerToPlayer(ply, "tooltip_test_setup", {})
...
function Attachments:ActivateAttachmentSetup()
  ...
  if Attachments.activated == false then
