...
require('abilities/general/modifiers/modifier_inside_marble')
require('abilities/cu_chulain/modifiers/modifier_self_disarm')
require('abilities/emiya/modifiers/modifier_hrunting_window')
...
function StartAnimation(unit, table)
  ...
  local bloop = table.bloop

  if unit.current_anim ~= nil then
    unit:RemoveGesture(unit.current_anim)
    Timers:RemoveTimer('anim' .. unit:GetName())
    Timers:CreateTimer(0.033, function()
      unit.current_anim = activity
      unit:StartGestureWithPlaybackRate(activity, rate)
      Timers:CreateTimer('anim' .. unit:GetName(), {
        endTime = duration,
        callback = function()
          unit:RemoveGesture(unit.current_anim)
          unit.current_anim = nil
      end})
    end)
  else
    unit.current_anim = activity
