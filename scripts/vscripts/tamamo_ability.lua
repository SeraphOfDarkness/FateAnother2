...
function OnSGApplyCC(keys)
	...
	giveUnitDataDrivenModifier(caster, target, "locked", keys.Duration)
...
function OnAmaterasuStart(keys)
	...
	if caster.IsTerritoryAcquired then 
		...
		local allies = FindUnitsInRadius(caster:GetTeam(), caster.AmaterasuCastLoc, nil, keys.Radius, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO, 0, FIND_ANY_ORDER, false) 
		for i = 1, #allies do
